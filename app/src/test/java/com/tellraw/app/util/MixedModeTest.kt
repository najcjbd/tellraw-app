...
class MixedModeTest {
    ...
    /**
     * 测试混合模式下移除普通的§m/§n（不带_f/_c后缀）
     */
    @Test
    fun testMixedModeRemovePlainMN() {
        val message = "§m普通删除线§n普通下划线"
        val bedrockJson = TextFormatter.convertToBedrockJson(message, "font", false)
        
        val rawtextContent = bedrockJson.substringAfter("text\":").substringBefore("}")
        // 普通的§m/§n应该被移除（因为混合模式下只处理§m_f/§m_c/§n_f/§n_c）
        assertFalse("基岩版不应包含独立的§m", rawtextContent.contains("§m"))
        assertFalse("基岩版不应包含独立的§n", rawtextContent.contains("§n"))
    }
}